...
GetOptions(	'version' => \$opt_version,
			...
			'dataname|dn=s' => \$dataname #CEB added
...
if ($loc_cutoff) {
	...
	open (VCFTEMP, ">", "$dataname.$cutoffs.temp.vcf") or die $!;
	my @lines;
	my $last_id = '';
	my $loc_removed = 0;
	my $snps_removed = 0;
	...
	$vcffile = "$dataname.$cutoffs.temp.vcf";
...
foreach my $ind (@samples) {
	...
	if ($threads) {
		$pm->start and next;
		open(TEMP, ">", "$ind.$cutoffs.haps");
		open(IFAIL, ">", "$ind.$cutoffs.fail.log");
		if ($debug) {
			open(LOG, ">", "$ind.$cutoffs.haps.log");
...
if ($threads) {
	...
	foreach my $ind (@samples) {
		...
		open(TEMPIN, "<", "$ind.$cutoffs.haps") or die $!;
		while(<TEMPIN>) {
			my ($locus, @haps) = split;
			$comb_haps{$locus}{$ind} = \@haps;
		}
		close TEMPIN;
		unlink "$ind.$cutoffs.haps";

		# Read the individual fail logs into the %failed hash
		open(FAILIN, "<", "$ind.$cutoffs.fail.log") or die $!;
		while(<FAILIN>) {
			chomp;
			my ($locus, $code) = split;
		 ...
		 close FAILIN;
		 unlink "$ind.$cutoffs.fail.log";

		 if ($debug) {

 			# Print the individual log files to a single file
 			open(LOGIN, "<", "$ind.$cutoffs.haps.log") or die $!;
 			while(<LOGIN>) {
 				print LOGOUT $_;
 			}
 			close LOGIN;
 			unlink "$ind.$cutoffs.haps.log";
 		}
...
if ($genepop) {
	...
	open(LOCI, '>', "$dataname.$cutoffs.hap_loci.txt") or die $!;
...
if ($imafile) {
	...
	write_ima(\%haplotypes, \%snps, $reference, \@samples, $imafile, $popmap, $dataname, $cutoffs);
}


# Print out some stats files for the run
open(STATS, ">", $dataname . '.' . $cutoffs . '.stats.out') or die $!;	#CEB added $dataname $cutoffs.
print STATS $command, "\n";
...
open(INDOUT, ">", $dataname . '.' . $cutoffs . '.ind_stats.out') or die $!;  #CEB added $cutoffs.
...
sub write_ima {
	...
	open(IMA, ">", "$dataname.$cutoffs.ima.temp") or die $!;
	...
	open(IMIN, "<", "$dataname.$cutoffs.ima.temp") or die $!;
	...
	foreach my $locus (keys %file) {
		...
		foreach my $pop (sort keys %{$file{$locus}}) {
			foreach my $ind (keys %{$file{$locus}{$pop}}) {
				my $ind_name = $ind;
				if (length($ind) > 10) {
				#CEB modify this line to change the sequence name
					$ind_name = substr($ind, -19, 9);
				}
...
Options:
	 ...
	 -bp		path to bam files, start from root
	 
	 -co		cutoffs from dDocent_HPC assembly, cutoff1.cutoff2
...
Absolute path to bam files if they are not in the current working directory (optional)

=item B<-co, --cutoffs>

Cutoffs from the assembly step of dDocent. Setting this option enables compatibility with HPC version of dDocent made by CEB (optional, don't set if you used the official dDocent)
